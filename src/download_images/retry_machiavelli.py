"""
Retry image search for Machiavelli with expanded custom queries including "The Prince".
"""
import sys
...
def main():
    """Re-search for Machiavelli with custom queries."""
    print("="*80)
    print("Searching for MACHIAVELLI with expanded 'The Prince' queries")
    print("="*80)
    ...
    # Fetch Machiavelli
    response = client.table('character').select('*').eq('name', 'MACHIAVELLI').execute()

    if not response.data:
        print("Error: Machiavelli not found in database")
        sys.exit(1)
    ...
    def custom_build_queries(char):
        """Build queries with custom searches for MACHIAVELLI."""
        custom_queries = get_custom_queries(char.name)
        if custom_queries:
            print(f"\n  Using {len(custom_queries)} custom search queries")
            print("  Including: 'The Prince', 'Niccolò Machiavelli', 'Renaissance Florence'")
